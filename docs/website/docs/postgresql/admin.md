...
```bash
cat <<EOF | sudo tee /etc/profile.d/node.sh
#!/bin/bash
#==============================================================#
# Environment
export EDITOR="vi"
...
# username: dba
groupadd -g 88 admin
useradd -u 88 -d /home/dba -g admin -G admin -m dba -s /bin/bash
mkdir -p /home/dba/.ssh
ssh-keygen -t rsa -b 4096 -C "dba@kubegg.io" -f /home/dba/.ssh/id_rsa -N ""
chown -R dba:admin /home/dba

# grant admin group nopass sudo
cat <<EOF | sudo tee /etc/sudoers.d/dba
%admin ALL=(ALL) NOPASSWD: ALL
EOF

chmod 0440 /etc/sudoers.d/dba

# touch admin user ssh config
cat <<EOF | sudo tee /home/dba/.ssh/config
...
chown dba:admin /home/dba/.ssh/config && \
chmod 0600 /home/dba/.ssh/config

sed -i 's/^StrictHostKeyChecking=.*/StrictHostKeyChecking=no/g' /home/dba/.ssh/config

# exchange all admin ssh keys among cluster
# e.g. 10.1.1.11
# check 'PubkeyAuthentication yes' /etc/ssh/sshd_config from 10.1.1.11
ssh-copy-id -i /home/dba/.ssh/id_rsa.pub root@10.1.1.11
ssh-copy-id -i /home/dba/.ssh/id_rsa.pub root@10.1.1.31
# ...other nodes
```
